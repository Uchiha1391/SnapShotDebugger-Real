...
namespace Assets.Editor
{
    static class SnapshotInjector
    {
        ...
        private static bool CompilerMessagesContainError(
        ...
        public static void InjectCode()
        {
            ...
            {
                var TypeDefinitions = AssemblyDefinitionInstance.MainModule
                    .GetTypes()
                    .Where(definition =>
                    {
                        ...
                        foreach (var CustomAttribute in att)
                            if (CustomAttribute.AttributeType.Name ==
                                nameof(SnapShotAttributes
                                    .SnapShotInjectionAttribute))
                                return true;
                ...
                foreach (var TypeDefinition in TypeDefinitions)
                {
                    bool CheckforAttributes(MethodDefinition definition)
                    {
                        ...
                        foreach (var CustomAttribute in at)
                            if (CustomAttribute.AttributeType.Name ==
                                nameof(SnapShotAttributes
                                    .IgnoreSnapShotInjectionAttribute))
                                return false;
                    ...
                    {
                        var IsMethodEligible =
                            CheckforAttributes(MethodDefinition);
                ...
                var InstructionsOfMethodToinjectList =
                    InstructionsOfMethodToinjectEnurmator.Body.Instructions
                        .ToList();
                ...
                if (MethodsTOinject != null)
                {
                    var FilterMethodList =
                        FilterMethodDefinitions(MethodsTOinject.ToList());
...

                    {
                        ...
                        ParametersInstructions.Add(IlProcessor.Create(
                            OpCodes.Newarr,
                        ...
                        for (var i = 0;
                            i < MethodDefinition.Parameters.Count;
                            i++)
                        {
                            ParametersInstructions.Add(IlProcessor.Create(
                                OpCodes.Ldloc,
                            ...
                            if (MethodDefinition.Parameters[i].ParameterType
                                .IsValueType)
                                ParametersInstructions.Add(IlProcessor.Create(
                                    OpCodes.Box,
                                ...
                                ParametersInstructions.Add(IlProcessor.Create(
                                    OpCodes.Castclass,
                                    AssemblyDefinitionInstance.MainModule
                                        .TypeSystem
                                        .Object)); // casting for reference types

                            ParametersInstructions.Add(
                                IlProcessor.Create(OpCodes
                                    .Stelem_Ref)); // store in the array
                        }
                        ...
                        FinalInstructionsToinject.Add(IlProcessor.Create(
                            OpCodes.Call,
                        ...
                        FinalInstructionsToinject.AddRange(
                            ParametersInstructions);

                        //
                        FinalInstructionsToinject.AddRange(
                            InstructionsOfMethodToinjectList);
                        ...
                        ) // add the new instructions in referse order
                        {
                            var firstInstruction =
                                MethodDefinition.Body.Instructions[0];
                            var processor =
                                MethodDefinition.Body.GetILProcessor();
        ...
        public static void testingEs3Generate()
        {
            ...
            CustomEs3TypeGeneratorData.Instance.fields =
                ES3Reflection.GetSerializableMembers(TypeToCreateScriptFor,
                    false);
            ...
            CustomEs3TypeGeneratorData.Instance.GenerateCustom(
                TypeToCreateScriptFor);
